...
author: "Hung-Wen Yeh, Katie Forthman"
date: "3/10/2020"
output: 
  html_document:
    highlight: tango
...
# ??? Marks a place where the documentation needs to be expanded
# !!! Marks a place where an error may occur and the code needs correction.
...
## 0.4 A brief overview of GFA
From the paper *Group Factor Analysis*, 2014, by Klami et al.
The group factor analysis problem is as follows: Assume a collection of observations $y_i ∈ \mathbb{R}^D$ for $i = 1,...,N$ collected in a data matrix $Y ∈ \mathbb{R}^{N×D}$, and a disjoint partition of the $D$ variables into $M$ groups {$G_m$}. The GFA task is to find a set of $K$ factors that describe $Y$ so that relationships between the groups can be separated from relationships within the groups. For notational simplicity, assume that the first $D_1$ variables correspond to the first group $G_1$, the following $D_2$ variables to $G_2$, and so on. Then we can write $Y = [X^{(1)}, ..., X^{(M)}]$, where $X^{(m)}$ is a subset of the data corresponding to $G_m$. We use $x_i^{(m)}$ to denote the $i$th sample (row) of $X^{(m)}$. Throughout
this paper we use the superscript $(m)$ to denote variables related to the $m$th group or data set.

A general solution to the GFA problem can be formulated as a joint factor model for the observed data sets. The model for the mth group of the ith sample is
$$\textbf{x}_i^{(m)} \sim \mathcal{N}(\textbf{W}^{(m)⊤} \textbf{z}_i, \tau_m^{-1} \textbf{I})$$
where $W^{(m)⊤} = [w_1^{(m)}, ..., w_K^{(m)}], z_i∈ \mathbb{R}^K$ and $\tau_m$ is noise precision. Equivalently, we can directly write $y_i = W^⊤ z_i + ε_i$, where $ε_i$ is Gaussian noise with diagonal covariance but separate variance for each group, by denoting $W = [W^{(1)},...,W^{(M)}]$.

# 1. (N,D,K,sd.noise) = (100, 40, 7, 0)
Sample size, N = 100
Number of variables, D = 40
Number of factors, K = 7
Standard deviation of noise = 0 (i.e. no noise added to simulation)

## 1.1. Simulate data
Here we essentially reverse the results of a GFA analysis (the W matrix) to derive observed data. This way, we can test the GFA function to see if we can correctly recover the posterior matrix.

### 1.1.0 Set up blocks and grids.
```{r}
...
### 1.1.1 Create the posterior matrix, W
Here we create a W matrix, which will be the desired output after performing GFA on our simulated data. The matrix is filled with random values. For each of the `K` factors, the blocks that load into that factor are specified by K.blocks. This simulated matrix is used to demonstrate how to use optmThrGFA.
```{r}
set.seed(123)
K <- 7
D <- sum(block.length)
...
### 1.1.2
Derive the observed data, Y, from the posterior matrix.
```{r}
...
This is simply a check that the variance explained by the generated data adds up to 1.
```{r eval = T}
# Increase sample size for total ve -> 1
veTrue <- matrix(nrow = K, ncol = 1)

# veTrue is the variance explained by each factor, takes into acct factor score variance of observed data
for(k in 1:K){ 
  veTrue[k] <- 
    mean(
      diag(
...
# diag(W^2) is the variance explained by each factor
apply(W^2, 2, mean)
sum(apply(W^2, 2, mean))

# factor score standard deviation of observed data
round(apply(dat$X, 2, sd), 2)
```

## 1.2 Replicate GFA
Perform GFA 10 times on the simulated data.
```{r eval = T}
...
Along with the GFA results are saved some useful information about each replicate. Here, we look at the convergence value (desired to be <0.05) and the inferred number of factors (desired K = `K`). Recall that no noise was added to the simulated data. Therefore, the GFA should have worked quite well.
The table produced shows that all 10 replicates converged and returned the correct K:
```{r}
...
for(r in 1:R){
  load(paste0("res/", folder, "/GFA_rep_", r, ".rda"))
  gfaList_full[[r]] <- res
  rep.summ$conv[r] <- res$conv
  rep.summ$K[r] <- res$K
}

# The convergence value should be below 0.05 to indicate the model has converged.
# Below is shown a summary of the convergence values of the replicates.
summary(rep.summ$conv)

# This is a table showing the convergence values of all replicates, as well as the inferred number of factors.
...
# file name
match.filename <- paste0("res/", folder, "/match_orig.rda")
...
if(!file.exists(match.filename)){
  ...
  for(i in 1:length(corGrids)){
    for (j in 1:length(matchGrids)){
      ...
      this.filename <- paste0("res/", folder, "/rComp_corThr", corGrids[i], "_matchThr", matchGrids[j], ".rda")
...
```{r}
load(paste0("res/", folder, "/match_orig.rda"))
...
```{r}
load(paste0("res/", folder, "/rComp_corThr0.3_matchThr0.6.rda"))
rcomp$indices
```

### 1.3.2. Using a list of posterior means of reconstructed data, lower memory required
Different method, compare to 1.3.1

Compute factor-specific posterior mean of reconstructed data (X*t(W)).
Runs pmXW_by_factor() on all replicates then saves the output to "xw_by_rep_comp.rda".
xw is a list of GFA replicates with posterior medians
???
```{r eval = T}
xw_by_rep_comp.filename <- paste0("res/", folder, "/xw_by_rep_comp.rda")
if(!file.exists(xw_by_rep_comp.filename)){
  xw <- list()
  for(r in 1:R){
    load(paste0("res/", folder, "/GFA_rep_", r, ".rda"))
...
```{r eval = T}
load(paste0("res/", folder, "/xw_by_rep_comp.rda"))

this.filename <- paste0("res/", folder, "/MSE_xwList.rda")
if(!file.exists(this.filename)){
  system.time(
    match.mse <- MSE.Grids(
  ...
  save(match.mse, file = this.filename)
}
```

Numbers of matched factors for each (corThr, matchThr) combination:
```{r}
load(paste0("res/", folder, "/MSE_xwList.rda"))
match.mse$K.grid
```

Both criteria matched 7 factors
```{r}
# optimal # of factors, pick best thresholds
...
```{r eval = T}
gfaList.filename <- paste0("res/", folder, "/gfaList_p50.rda.rda")
if(!file.exists(gfaList.filename)){
  gfaList_p50 <- list()
  for (r in 1:R){ 
...
```{r}
load(paste0("res/", folder, "/gfaList_p50.rda.rda"))
rob.ind <- list(indices=match_orig$indices[[1]][[4]])
ve_full <- rob.var.exp(models=gfaList_p50, rob=rob.ind, block.names=block.names, varIdx.by.block=varIdx.by.block, use.unmatched=T, by.block=T)
```
...
save(rWX_full, file=paste0("res/", folder, "/rWX_full.rda"))
```

```{r eval = T}
load(paste0("res/", folder, "/rWX_full.rda"))
...
for(i in 1:length(corGrids)){
  for (j in 1:length(matchGrids)){
    rcomp <- robustComponents(gfaList_full, corThr=corGrids[i], matchThr=matchGrids[j])
    save(rcomp, file=paste0("res/", folder, "/rComp_corThr", corGrids[i], "_matchThr", matchGrids[j], ".rda"))
    match_orig$K.girds[i,j] <- rcomp$Krobust
    yhat <- apply(rcomp$effect, 1:2, sum, na.rm=T)
    match_orig$mse.m[i,j] <- mean((yhat - Y)^2)
  }
}
save(match_orig, file=paste0("res/", folder, "/match_orig.rda"))
```
...
```{r}
load(paste0("res/", folder, "/rComp_corThr0.6_matchThr0.1.rda"))
rcomp$indices
```
...
rep.summ[rep.summ$conv>0.1,]
xw <- xw[-4]
save(xw, file=paste0("res/", folder, "/xw_by_rep_comp.rda"))
```
...
save(match.mse, file=paste0("res/", folder, "/MSE_xwList.rda"))
```
...
foreach(r = 1:15, .packages=c("GFA")) %dopar% {
  ...
  save(res, file=paste0("res/", folder, "/GFA_rep_", r, ".rda"))
}
```
...
for(i in 1:length(corGrids)){
  for (j in 1:length(matchGrids)){
    rcomp <- robustComponents(gfaList_full, corThr=corGrids[i], matchThr=matchGrids[j])
    save(rcomp, file=paste0("res/", folder, "/rComp_corThr", corGrids[i], "_matchThr", matchGrids[j], ".rda"))
    # load(paste0("res/", folder, "rComp_corThr", corGrids[i], "_matchThr", matchGrids[j], ".rda"))
    match_orig$K.girds[i,j] <- rcomp$Krobust
    yhat <- apply(rcomp$effect, 1:2, sum, na.rm=T)
    match_orig$mse.m[i,j] <- mean((yhat - Y)^2)
  }
...
save(gfaList_p50, file=paste0("res/", folder, "/gfaList_p50.rda.rda"))
```
...
save(rWX_p50.min, file=paste0("res/", folder, "/rWX_p50.min.rda"))
```

```{r}
load(paste0("res/", folder, "/rWX_p50.min.rda"))
...
save(rWX_p50.1se, file=paste0("res/", folder, "/rWX_p50.1se.rda"))
```

```{r}
load(paste0("res/", folder, "/rWX_p50.1se.rda"))
